...
main() {
    ...
    if [ $? -eq 1 ]; then
        ...
        touch "$updatedir/.JukaMixUpdateAvailable"
        sync
    fi
    echo -ne "${YELLOW}"
    ...
    if [ -f "$updatedir/JukaMix_$Release_Version.zip" ] || [ -f "/mnt/SDCARD/JukaMix_$Release_Version.zip" ]; then

        if [ ! -f "$updatedir/JukaMix_$Release_Version.zip" ] && [ -f "/mnt/SDCARD/JukaMix_$Release_Version.zip" ]; then
            echo -e "File found in SDCARD root,\nmoving to $updatedir"
            mv "/mnt/SDCARD/JukaMix_$Release_Version.zip" "$updatedir/"
            sleep 4
        fi
        echo -e "\nJukaMix_$Release_Version.zip already present,\nchecking file size:"
        check_update
        if [ $? -eq 1 ]; then
            sleep 5
            download_update
...
get_release_info() {
    ...
    if echo "$Release_assets_info" | grep -q '"message": "Not Found"'; then
        rm "$updatedir/.JukaMixUpdateAvailable" 2>/dev/null
        echo -e "${GREEN}DONE${NC}\n\nNo update available for $channel channel\n"
        sync
        return 1
    fi

    Release_asset=$(echo "$Release_assets_info" | jq '.assets[]? | select((.name | startswith("JukaMix_")) and (.name | endswith(".zip")))')
    Release_url=$(echo $Release_asset | jq '.browser_download_url' | tr -d '"')
    Release_FullVersion=$(echo $Release_asset | jq '.name' | tr -d "\"" | sed 's/^JukaMix_//g' | sed 's/\.zip$//g')
    ...
    Local_Version=$(echo $Local_JukaMixVersion | sed 's/-dev.*$//g')

    echo -e "${GREEN}DONE${NC}"

    echo -ne "\n\n\n\n" \
        "${BLUE}======= Installed Version ========${NC}\n" \
        " Version: $Local_JukaMixVersion \n" \
        "${BLUE}==================================${NC}\n"
    echo -ne "\n\n" \
    ...
    if [ "$Local_Version" -gt "$Remote_Version" ] || { [ "$Local_Version" -eq "$Remote_Version" ] && [ "$Local_JukaMixVersion" = "$Release_FullVersion" ]; }; then
        echo -e "No new major version of JukaMix OS available.\n"
        rm "$updatedir/.JukaMixUpdateAvailable" 2>/dev/null
        sync
        return 1
    fi
...
download_update() {
    Mychoice=$(echo -e "No\nYes" | /mnt/SDCARD/System/usr/trimui/scripts/shellect.sh -t "Download $Release_Version (${Release_size_MB}MB) ?" -b "Press A to validate your choice.")
    ...
    if [ "$Mychoice" = "Yes" ]; then
        ...
        clear
        check_filesystem
        rm "$updatedir/JukaMix_$Release_Version.zip" 2>/dev/null
        download_file $Release_url -f "$updatedir/JukaMix_$Release_Version.zip" -t "Downloading JukaMix OS $Release_Version"
...
check_update() {
    ...
    Downloaded_size=$(stat -c %s "$updatedir/JukaMix_$Release_Version.zip")
...
apply_update() {
    ...
    if [ "$Mychoice" = "Yes" ]; then
        ...
        UpdateScript="https://raw.githubusercontent.com/jukaLang/JukaMix-OS/refs/tags/JukaMix_$Release_Version/System/usr/trimui/scripts/jukamix_update.sh"
        DownloadPath="/mnt/SDCARD/System/usr/trimui/scripts/jukamix_update.sh"
        ...
        mv "$updatedir/JukaMix_$Release_Version.zip" "/mnt/SDCARD/"
