...
check_connection() {
    ...
    if wget -q --spider https://github.com >/dev/null; then
        ...
        /usr/sbin/ntpd -p ntp5.aliyun.com -p 0.openwrt.pool.ntp.org -n -q

        echo -n "Retrying internet connection... "
        if wget -q --spider https://github.com >/dev/null; then
            echo -e "${GREEN}OK${NC}"
            return 0
        else
            echo -e "${RED}FAIL${NC}\nError: https://github.com not reachable. Check your wifi connection."
            echo -ne "${YELLOW}"
            [ "$silent" -eq 0 ] && infoscreen.sh -m "Connection failed check your connection and current date." -t 0.5
            return 1
        fi
    fi
}
...
enable_wifi() {
    ...
    # Wait up to ~10s for an IP address
    IP=""
    _attempts=0
    while [ "$_attempts" -lt 10 ] && [ -z "$IP" ]; do
        sleep 1
...
check_available_space() {
    ...
    if [ "$available_space" -lt "$required_space" ]; then
        ...
        echo "JukaMix OS update requires $required_space MB of free space."
        return 1
    else
