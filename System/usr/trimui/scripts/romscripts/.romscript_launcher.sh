...
# POSIX global text replacement (${var//...} is a bashism and busybox ash has no
# parameter-pattern substitution). Usage: replace_all <text> <pattern> <replacement>
replace_all() {
    _in=$1
    _pat=$2
    _rep=$(printf '%s' "$3" | sed 's/[&\\/]/\\&/g')
    printf '%s' "$_in" | sed "s/$_pat/$_rep/g"
}
...
add_choice() {
    ...
    display_name=$(replace_all "$display_name" "EMU_DIR" "$EMU_DIR_BASENAME")
    display_name=$(replace_all "$display_name" "PRESET_TYPE" "$Preset_Type")
...
if [ -d "$SCRIPT_DIR" ]; then
    ...
    for subdir in "$SCRIPT_DIR"/*/; do
        ...
        display_header=$(replace_all "$display_header" " LAUNCHER_NAME" ": $Launcher_name")
        display_header=$(replace_all "$display_header" " PRESET_TYPE" ": $Preset_Type")
...
# Show selector menu
eval "selector_output=\$(selector -i /mnt/SDCARD/trimui/res/jukamix-os/bg-plain.png -t \"Choose an action for $ROM_FILENAME_NOEXT\" -fs 150 -c $choices)"
