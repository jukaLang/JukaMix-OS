...
# Update theme pack in JukaMix OS configuration
if [ -n "$packname" ] && [ -n "$style" ]; then
    jq --arg packname "$packname" --arg style "$style" '.["THEME PACK"] = $packname | .["JUKAMIX STYLE"] = $style' /mnt/SDCARD/System/etc/jukamix.json >/tmp/jukamix.json && mv /tmp/jukamix.json /mnt/SDCARD/System/etc/jukamix.json
else
...
if [ -n "$theme" ]; then
    if [ -d "/mnt/SDCARD/Themes/${theme}" ]; then
        ...
        [ "$theme" = "JukaMix - OS" ] && theme="Default"
...
if [ -n "$packname" ]; then
    if [ -f "/mnt/SDCARD/trimui/res/jukamix-os/theme_$packname/icon.ico" ]; then
        /mnt/SDCARD/System/usr/trimui/scripts/set_ra_cfg.sh /mnt/SDCARD/autorun.inf icon "trimui/res/jukamix-os/theme_$packname/icon.ico"
    else
        /mnt/SDCARD/System/usr/trimui/scripts/set_ra_cfg.sh /mnt/SDCARD/autorun.inf icon "trimui/res/jukamix-os/icon.ico"
    fi
fi
