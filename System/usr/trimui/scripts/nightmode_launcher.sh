...
if [ "$1" = "osd" ]; then
    NightMode=$(/mnt/SDCARD/System/bin/jq -r '.["NightMode_OSD"]' "/mnt/SDCARD/System/etc/jukamix.json")
else
    NightMode=$(/mnt/SDCARD/System/bin/jq -r '.["NightMode"]' "/mnt/SDCARD/System/etc/jukamix.json")
fi
...
if [ "$NightMode" = "Configurator" ]; then
    ...
    echo "NightMode feature disabled in jukamix.json"
