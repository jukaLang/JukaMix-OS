#!/bin/sh
# Silent background update checker for JukaMix OS
...
check_major_release() {
    ...
    asset=$(echo "$release_json" | jq '.assets[]? | select((.name | startswith("JukaMix_")) and (.name | endswith(".zip")))')
    if [ -z "$asset" ]; then
        log "Github API Request has failed or latest release has no JukaMix_*.zip asset."
        rm -f "$RELEASE_META"
        exit 1
    fi
    ...
    release_full_version="${name#JukaMix_}"
    release_full_version="${release_full_version%.zip}"
    remote_version_core="${release_full_version%-dev*}"

    local_full_version=$(cat /mnt/SDCARD/System/usr/trimui/jukamix-version.txt 2>/dev/null)
    ...
    if [ "$local_v" -lt "$remote_v" ] || { [ "$local_v" -eq "$remote_v" ] && [ "$local_full_version" != "$release_full_version" ]; }; then
        ...
        echo -e "{ \"type\":\"info\", \"size\":2, \"duration\":5000, \"x\":670, \"y\":640,  \"message\":\"       JukaMix $release_full_version available.\",  \"icon\":\"\" }" >/tmp/trimui_osd/osd_toast_msg
...
# Check hotfix availability for a given base version (a date stamp, e.g. 0820202614)
check_hotfix() {
    base_version="$1"
    url="https://raw.githubusercontent.com/$GITHUB_REPOSITORY/main/_assets/hotfixes/JukaMix_${base_version}.sh"
    ...
    if [ -f /mnt/SDCARD/System/usr/trimui/jukamix-hotfix-version.txt ]; then
        local_hotfix=$(cat /mnt/SDCARD/System/usr/trimui/jukamix-hotfix-version.txt)
    else
...
main() {
    ...
    base_version=$(cat /mnt/SDCARD/System/usr/trimui/jukamix-version.txt 2>/dev/null)
    ...
    if check_hotfix "$local_version_core"; then
        ...
        echo -e "{ \"type\":\"info\", \"size\":2, \"duration\":5000, \"x\":670, \"y\":640,  \"message\":\"JukaMix OS Hotfix $remote_hotfix available.\",  \"icon\":\"\" }" >/tmp/trimui_osd/osd_toast_msg
        exit 0
    fi
