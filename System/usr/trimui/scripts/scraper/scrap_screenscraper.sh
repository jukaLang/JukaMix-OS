...
for file in $(eval "find /mnt/SDCARD/Roms/$CurrentSystem -maxdepth 2 -type f \
    ...
    if [ -f "/mnt/SDCARD/Imgs/$CurrentSystem/$romNameNoExtension.png" ]; then
        ...
        url="https://www.screenscraper.fr/api2/jeuInfos.php?devid=${u%?}&devpassword=${p#??}&softname=jukamix&output=json&ssid=${userSS}&sspassword=${passSS}&sha1=&systemeid=${ssSystemID}&romtype=rom&romnom=${romNameTrimmed}.zip&romtaille=${rom_size}"
        search_on_screenscraper
        ...
        if ! [ "$gameIDSS" -eq "$gameIDSS" ] 2>/dev/null; then
            ...
            if [ "$rom_size" -gt "$MAX_FILE_SIZE_BYTES" ]; then
                ...
                url="https://www.screenscraper.fr/api2/jeuInfos.php?devid=${u%?}&devpassword=${p#??}&softname=jukamix&output=json&ssid=${userSS}&sspassword=${passSS}&sha1=${checksum}&systemeid=&romtype=rom&romnom=${romNameTrimmed}.zip&romtaille=${rom_size}"
                search_on_screenscraper
                if ! [ "$gameIDSS" -eq "$gameIDSS" ] 2>/dev/null; then
