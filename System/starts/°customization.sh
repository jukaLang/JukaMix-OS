...
version=$(cat /mnt/SDCARD/System/usr/trimui/jukamix-version.txt)
...
FW_patched_version=$(cat /usr/trimui/jukamix-version.txt)
...
if [ "$version" != "$FW_patched_version" ]; then
    ...
    if [ "$JukaMix_Update" = "0" ]; then
        ...
        "/mnt/SDCARD/Emus/_BootLogo/launch.sh" "$src_dir/- JukaMix-OS.bmp"
    fi
fi
