...
JukaMixFWfile="/mnt/SDCARD/trimui/firmwares/MinFwVersion.txt"

if [ ! -f "$JukaMixFWfile" ]; then
    echo "Configuration file $JukaMixFWfile not found, skipping firmware check."
    exit 0
fi

Current_FW_Revision=$(grep 'DISTRIB_DESCRIPTION' /etc/openwrt_release | cut -d '.' -f 3)
Required_FW_Revision=$(sed -n '2p' "$JukaMixFWfile")
...
check_firmware_crc() {
    ...
    /mnt/SDCARD/System/usr/trimui/scripts/infoscreen2.sh -m "$message" -fs 12 -fi 0 -p top-left -fb -sp -ff "/mnt/SDCARD/Themes/JukaMix - OS/wqy-microhei.ttf" -i "/mnt/SDCARD/trimui/firmwares/FW_Screen_Wait.jpg" &
...
')
    ...
    if ! check_available_space "$(($FW_Size_MB + 100))"; then
        ...
        pkill presenter
        sleep 0.5
        /mnt/SDCARD/System/usr/trimui/scripts/infoscreen2.sh -m "$message" -fs 12 -fi 0 -p top-left -fb -ff "/mnt/SDCARD/Themes/JukaMix - OS/wqy-microhei.ttf" -i "/mnt/SDCARD/trimui/firmwares/FW_Screen_Fail.jpg" -k rout A "Exit"
    else
...
if [ -z "$Current_FW_Revision" ] || [ -z "$Required_FW_Revision" ]; then
    ...
    if [ "$Current_FW_Revision" -lt "$Required_FW_Revision" ]; then
        ...
        Required_FW_Version=$(sed -n '1p' "$JukaMixFWfile")

        echo "Current firmware ($Current_FW_Version - $Current_FW_Revision) must be updated to $Required_FW_Version - $Required_FW_Revision to support JukaMix OS v$JukaMix_version."
        ...
        if [ "$crc_verified" = false ]; then
            ...
            if ! /mnt/SDCARD/System/bin/7zz x "$FIRMWARE_PATH" -o"/mnt/SDCARD" -y; then
                ...
                /mnt/SDCARD/System/usr/trimui/scripts/infoscreen2.sh -m "$message" -fs 12 -fi 0 -p top-left -fb -ff "/mnt/SDCARD/Themes/JukaMix - OS/wqy-microhei.ttf" -i "/mnt/SDCARD/trimui/firmwares/FW_Screen_Fail.jpg" -k rout A "" -k rout X ""
        ...
        if [ "$crc_verified" = true ]; then
            ...
            /mnt/SDCARD/System/usr/trimui/scripts/infoscreen2.sh -m "$message" -fs 12 -fi 0 -p top-left -fb -ff "/mnt/SDCARD/Themes/JukaMix - OS/wqy-microhei.ttf" -i "/mnt/SDCARD/trimui/firmwares/FW_Screen_Ready.jpg" &
            sleep 1.5
            pkill presenter
            sleep 0.5
            message="${message}\nReady for update!\n \nPlease read instructions at the right\nto launch your firmware upgrade."
            /mnt/SDCARD/System/usr/trimui/scripts/infoscreen2.sh -m "$message" -fs 12 -fi 0 -p top-left -fb -ff "/mnt/SDCARD/Themes/JukaMix - OS/wqy-microhei.ttf" -i "/mnt/SDCARD/trimui/firmwares/FW_Screen_Ready.jpg" -k rout A "" -k rin B ""
...
################ check if a JukaMix OS update is available ################
...
if [ -n "$UPDATE_FILE" ]; then
    ...
    echo "JukaMix OS install file found: $UPDATE_FILE"
    initial_version=$(cat /mnt/SDCARD/System/usr/trimui/jukamix-version.txt)
    update_version=$(echo "$UPDATE_FILE" | awk -F'_|[.]zip' '{print $2}')

    # Compare the versions
    if [ "$(echo "$update_version" | tr -d '.')" -gt "$(echo "$initial_version" | tr -d '.')" ]; then
        echo "The JukaMix OS update file ($update_version) is greater than the current version installed ($initial_version)."
        ...
        if [ "$rootfs_space" -lt "$minspace" ]; then
            ...
            infoscreen.sh -m "JukaMix OS update $update_version found. Not enough space on internal storage to update." -k "A B START MENU" -fs 30
            exit 1
        else
            echo "Available space on / is sufficient: ${rootfs_space} KB"
        fi

        if [ ! -f "/mnt/SDCARD/trimui/res/jukamix-os/bg-info.png" ] ||
            [ ! -f "/mnt/SDCARD/System/bin/sdl2imgshow" ] ||
            [ ! -f "/mnt/SDCARD/System/usr/trimui/scripts/jukamix_update.sh" ] ||
            ...
            /mnt/SDCARD/System/bin/7zz -aoa x "$UPDATE_FILE" \
                ...
                -i"!trimui/res/jukamix-os/*"
            sync
        fi

        button=$(infoscreen.sh -m "JukaMix OS update v$update_version found. Press A to install, B to cancel." -k "A B")
        if [ "$button" = "A" ]; then
            ...
            infoscreen.sh -m "Updating JukaMix OS to v$update_version" -t 1
            pkill -9 preload.sh
            pkill -9 runtrimui.sh
            /tmp/text_viewer -s "/tmp/jukamix_update.sh" -f 25 -t "                            JukaMix OS Update v$update_version                                      "
        fi
    else
        echo "The JukaMix OS update version ($update_version) is not greater than the current version ($initial_version)."
    fi
else
    echo "No JukaMix OS update file found."
fi
