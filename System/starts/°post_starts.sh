...
eval $(
	/mnt/SDCARD/System/bin/jq -r '
        ...
        "' "/mnt/SDCARD/System/etc/jukamix.json"
)
...
# --- JukaHub auto-install ---
# Install the integrated JukaHub hub app once: when it is missing and the user
# hasn't opted out (System/var/jukamix/state/jukahub-skip). Runs in the
# background after Wi-Fi is up, so boot is never blocked; a failed download is
# simply retried on the next boot. Superseded Apps/ entries are moved into a
# recoverable backup (jukamix-jukahub.sh restore-apps brings them back).
if [ "$wifi_value" -eq 1 ] && [ ! -x /mnt/SDCARD/JukaHub/JukaHub ] && [ ! -f /mnt/SDCARD/System/var/jukamix/state/jukahub-skip ]; then
	(
		_i=0
		while [ "$_i" -lt 30 ]; do
		...
		[ -f /mnt/SDCARD/tools/jukamix-jukahub.sh ] && \
			sh /mnt/SDCARD/tools/jukamix-jukahub.sh install --replace-apps --background \
				>/tmp/jukahub-install.log 2>&1
	) &
fi
