...
if [ ! -f "/mnt/SDCARD/System/etc/jukamix.json" ]; then
  touch "/mnt/SDCARD/System/etc/jukamix.json"
fi
sync
...
if [ -d "/mnt/SDCARD/Apps/SystemTools/Menu/Imgs_$JukaMix_Style" ]; then
  img_path="/mnt/SDCARD/Apps/SystemTools/Menu/Imgs_$JukaMix_Style"
else
...
for subdir in /mnt/SDCARD/Backgrounds/*/; do
  ...
  if [ -f "${subdir}preview_$JukaMix_Style.png" ]; then
    # Copy themed preview_$JukaMix_Style.png with subfolder name
    cp "${subdir}preview_$JukaMix_Style.png" "${BG_imgs_directory}${subdir_name}.png"
...
for subdir in /mnt/SDCARD/Icons/*/; do
  ...
  if [ -f "${subdir}preview_$JukaMix_Style.png" ]; then
    # Copy preview.png with subfolder name
    cp "${subdir}preview_$JukaMix_Style.png" "${ICON_imgs_directory}${subdir_name}.png"
  elif [ -f "${subdir}preview.png" ]; then
    # Copy preview.png with subfolder name
...
if [ -d "${subdir}Apps" ]; then
  ...
  cp "${ICON_list_directory}Default.sh" "${ICON_list_directory}${subdir_name}.sh"
  # Check if preview.png file exists
  if [ -f "${subdir}preview_apps_$JukaMix_Style.png" ]; then
    # Copy preview.png with subfolder name
    cp "${subdir}preview_apps_$JukaMix_Style.png" "${ICON_imgs_directory}${subdir_name}.png"
...
for subdir in /mnt/SDCARD/Themes/*/; do
  ...
  if [ -f "${subdir}preview_$JukaMix_Style.png" ]; then
    # Copy preview.png with subfolder name
    cp "${subdir}preview_$JukaMix_Style.png" "${THEME_imgs_directory}${subdir_name}.png"
  elif [ -f "${subdir}preview.png" ]; then
    # Copy preview.png with subfolder name
  ...
  if [ "$subdir_name" = "JukaMix - OS" ]; then
...
sqlite3 "$database_file" "SELECT disp, path FROM Menu_roms WHERE type = 1 AND disp LIKE '% (state)' ;" |
  while IFS='|' read -r disp path; do
    ...
    CurState=$(jq -r --arg disp "$disp_withoutstate" '.[$disp]' "/mnt/SDCARD/System/etc/jukamix.json")
...
while IFS='|' read -r disp path; do
    ...
    if [ "$disp_withoutvalue" = "THEMES" ]; then
        ...
        CurState=$(jq -r --arg disp "$ConfigKey" '.[$disp] // "Default"' "/mnt/SDCARD/System/etc/jukamix.json")
    fi
