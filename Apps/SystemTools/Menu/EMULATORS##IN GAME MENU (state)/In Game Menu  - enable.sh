#!/bin/sh
export PATH="/mnt/SDCARD/System/usr/trimui/scripts/:/mnt/SDCARD/System/bin:${PATH:+:$PATH}"
export LD_LIBRARY_PATH="/mnt/SDCARD/System/lib:/usr/trimui/lib:$LD_LIBRARY_PATH"

infoscreen.sh -m "Applying \"$(basename "$0" .sh)\" by default..."

json_file="/mnt/SDCARD/System/etc/jukamix.json"

if [ ! -f "$json_file" ]; then
  echo "{}" >"$json_file"
fi

jq '. += {"IN GAME MENU": 1}' "$json_file" >"/tmp/json_file.tmp" && mv "/tmp/json_file.tmp" "$json_file"

rm -f "/tmp/trimui_ra64/disable_tmenu"

# we modify the DB entries to reflect the current state
mainui_state_update.sh "IN GAME MENU" "enabled"
