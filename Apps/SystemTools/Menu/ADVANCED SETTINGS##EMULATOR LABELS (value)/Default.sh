...
for dir in /mnt/SDCARD/Emus/*/; do
  ...
  if [ -f "$config_file" ]; then
    # Retrieve the jukamix_name from the database
    jukamix_name=$(sqlite3 "$db_path" "SELECT jukamix_name FROM systems WHERE jukamix_foldername = '$folder_name' LIMIT 1")
    if [ -n "$jukamix_name" ]; then
      ...
      echo "No jukamix_name found for folder $folder_name"
    fi
  fi
done
