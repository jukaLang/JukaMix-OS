...
for dir in /mnt/SDCARD/Emus/*/; do
  ...
  if [ -f "$config_file" ]; then
    ...
    trimui_name_short_US=$(sqlite3 "$db_path" "SELECT trimui_name_short_US FROM systems WHERE jukamix_foldername = '$folder_name' LIMIT 1")
    if [ -n "$manufacturer" ] && [ -n "$trimui_name_short_US" ]; then
      # Construct the jukamix_name
      first_three_letters=$(echo "$manufacturer" | cut -c1-3)
      jukamix_name="$first_three_letters. $trimui_name_short_US"
