...
for dir in /mnt/SDCARD/Emus/*/; do
  ...
  if [ -f "$config_file" ]; then
    ...
    manufacturer=$(sqlite3 "$db_path" "SELECT manufacturer FROM systems WHERE jukamix_foldername = '$folder_name' LIMIT 1")
    trimui_name_short_EU=$(sqlite3 "$db_path" "SELECT trimui_name_short_EU FROM systems WHERE jukamix_foldername = '$folder_name' LIMIT 1")
    if [ -n "$manufacturer" ] && [ -n "$trimui_name_short_EU" ]; then
      # Construct the jukamix_name
      first_three_letters=$(echo "$manufacturer" | cut -c1-3)
      jukamix_name="$first_three_letters. $trimui_name_short_EU"
      # Update the label value in the JSON file
      /mnt/SDCARD/System/bin/jq --arg new_label "$jukamix_name" '.label = $new_label' "$config_file" > /tmp/tmp_config.json && mv /tmp/tmp_config.json "$config_file"
      echo "Updated label in $folder_name to \"$jukamix_name\""
    else
...
# Check for the existence of jukamix.json and update it
